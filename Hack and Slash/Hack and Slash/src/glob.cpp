...
void Glob::update() {
	...
	think();
	//TODO put this functions in parent class
	updateCollisionBox();
	updateMovement();
	updateCollisions();
	updateHitBox();
...
void Glob::findNearestTarget() {
	...
	for (auto entity = Entity::entities.begin(); entity != Entity::entities.end(); entity++) {
		if ((*entity)->type == "hero" && (*entity)->active) {
			...
			if (target == NULL) {
				target = dynamic_cast<LivingEntity*>((*entity)); // if cant cast to livingentity, throw casting exception
			}
			else {
